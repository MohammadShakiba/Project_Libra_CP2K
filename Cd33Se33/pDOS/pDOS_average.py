...
# In fact the k1 is Cd, k2 is Se
# So if we do range(1,3) or [1,2] it will start with the order of Cd and then Se
# If we choose [2,1] it will first start with Se and then Cd angular momentum columns
for k in range(1,3):
#for k in [2,1]:
...
plt.xlim(-5,5)
plt.ylim(0,40)
plt.legend(fontsize=6.75,ncol=2)
plt.xlabel('Energy, eV',fontsize=12)
plt.ylabel('DOS, 1/eV',fontsize=12)
plt.title("Cd$_{33}$Se$_{33}$ pDOS",fontsize=12)
plt.tight_layout()
plt.show()

plt.savefig('DOS_average_Cd33Se33_'+outname+'_average.png', dpi=300)
