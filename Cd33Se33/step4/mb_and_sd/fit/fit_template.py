...
if dynamics_option == 0:
    ...
    r_squared_tol = 0.8
    istates = [ [25,26,27,28], [25,26,27,28], [6,7,8], [14,15,16] ]
# Recovery
...
for decoherence_option in decoherence_options:
    ...
    for basis_option in basis_options:
        ...
        for subtraj in range(nsubtrajs):
            ...
            for istate in istates[basis_option_count]:
                ...
                filename = "../run_namd/_out_cd33se33_"+basis_option+"_nbra_namd_"+decoherence_option+"_"+str(subtraj)+"_"+str(istate)+".txt"
                ...
                for i in range(sz):
                    ...
                    if dynamics_option == 0:
                        ...
                        # mixed_sd dynamics
                            ...
                            y = float( namd_data_line[ 191 ] ) - float( namd_data_line[4] )
                            if y < 0:
                                y = 0.00

                        # elec dynamics
                        elif basis_option_count == 2:
                            y = float( namd_data_line[ 44 ] ) - float( namd_data_line[4] )
                            if y < 0:
                                y = 0.00

                        # hole dynamics
                        elif basis_option_count == 3:
                            y = float( namd_data_line[ 77 ] ) - float( namd_data_line[4] )
                            if y < 0:
                                y = 0.00
                    ...
                    # Recovery
                        ...
                        if basis_option_count == 0 or basis_option_count == 1:
                            population_recovered = float( namd_data_line[ 3 ] ) + float( namd_data_line[ 6 ] ) + float( namd_data_line[ 9 ] ) + float( namd_data_line[ 12 ] ) + float( namd_data_line[ 15 ] ) + float( namd_data_line[ 18 ] ) + float( namd_data_line[ 21 ] )
    ...
    plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
    plt.subplot(1,1,1)
    plt.title("(CdSe)$_{33}$ "+decoherence_options_names[decoherence_option_count], fontsize=12)
    ...
    if dynamics_option == 0:
        plt.savefig("Cd33Se33_mb_mixed_sd_Hot_Energy_Decay_tp0_"+decoherence_options[decoherence_option_count])
    elif dynamics_option == 1:
        plt.savefig("Cd33Se33_mb_mixed_sd_Recovery_tp0_"+decoherence_options[decoherence_option_count])
    ...
    if dynamics_option == 0:
        plt.savefig("Cd33Se33_elec_hole_Hot_Energy_Decay_tp0_"+decoherence_options[decoherence_option_count])
    elif dynamics_option == 1:
        plt.savefig("Cd33Se33_elec_hole_sd_Recovery_tp0_"+decoherence_options[decoherence_option_count])
