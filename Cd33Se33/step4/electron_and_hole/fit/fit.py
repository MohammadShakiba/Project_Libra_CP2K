...
for decoherence_option in decoherence_options:
    ...
    for basis_option in basis_options:
        ...
        for subtraj in range(nsubtrajs):
            ...
            for istate in istates[basis_option_count]:
                ...
                plt.subplot(1,1,1)
                plt.title("(CdSe)$_{33}$ traj "+str(subtraj)+""+basis_option+" basis, "+decoherence_option+" istate = "+str(istate)) 
    ...
    for i in range( niconds ):
        plt.plot( xdata, dyns[decoherence_option_count][0][i], label="", linewidth=1, color = "mediumorchid")
        plt.plot( xdata, dyns[decoherence_option_count][1][i], label="", linewidth=1, color = "mediumseagreen")
    plt.plot(xdata, ydata_avg_sd,     linewidth=2.5, color="black", label="")
    plt.plot(xdata, ydata_avg_mb,     linewidth=2.5, color="black", label="")
    plt.plot(xdata, ydata_avg_sd_fit, linewidth=1,   color="purple",   label="HOMO$\ rightarrow$ LUMO+N")
    plt.plot(xdata, ydata_avg_mb_fit, linewidth=1,   color="green",    label="HOMO-N$\ rightarrow$ LUMO")
    ...
    if dynamics_option == 0:
        plt.savefig("Cd33Se33_Hot_Energy_Decay_"+decoherence_options[decoherence_option_count])
    elif dynamics_option == 1:
        plt.savefig("Cd33Se33_Recovery_"+decoherence_options[decoherence_option_count])
