...
params = {}
params["data_set_paths"] = []
params["data_set_paths"].append(absolute_path+"/../../../step3/electron_only/res_sd/res_sd/")
params["Hvib_re_prefix"] = "Hvib_sd_"; params["Hvib_re_suffix"] = "_re"
params["Hvib_im_prefix"] = "Hvib_sd_"; params["Hvib_im_suffix"] = "_im"
params["nfiles"]         = 4000
...
def myfunc( subtraj ):
    ...
    for istate in istates:
        ...
        params["nstates"]  = 14 # total number of mb electronic states
        ...
        plt.subplot(1,1,1)
        plt.title('(CdSe)$_{33}$ HOMO -> LUMO+N traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',      fontsize=10)
        plt.ylabel('Energy, eV',    fontsize=10)
        plt.ylim(0,3.5)
        plt.yticks([0,1,2,3])
        for sd_index in range( 14 ):
        ...
        plt.savefig("CdSe33_elec_Dyn_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)
    ...
    for istate in istates:
        ...
        params["nstates"]  = 25  # total number of sd electronic states
        ...
        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('(CdSe)$_{33}$ HOMO-N -> LUMO traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',      fontsize=10)
        plt.ylabel('Energy, eV',    fontsize=10)
        plt.ylim(0,3.5)
        plt.yticks([0,1,2,3])
        for sd_index in range( 25 ):
        ...
        plt.tight_layout()
        plt.legend(fontsize=8, ncol=3, loc="lower left")
        plt.savefig("CdSe33_hole_Dyn_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)
