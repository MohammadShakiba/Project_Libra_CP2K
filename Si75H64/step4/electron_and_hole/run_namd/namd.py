...
print ("Length of hvib_mb_traj1 is: ", len(hvib_mb[0]))
#sys.exit(0)
...
params = {}
params["data_set_paths"] = []
params["data_set_paths"].append(absolute_path+"/../../../step3/hole_only/res_sd/res_sd/")
params["Hvib_re_prefix"] = "Hvib_sd_"; params["Hvib_re_suffix"] = "_re"
params["Hvib_im_prefix"] = "Hvib_sd_"; params["Hvib_im_suffix"] = "_im"
params["nfiles"]         = 4000
...
print ("Length of hvib_sd_traj1 is: ", len(hvib_sd[0]))
#sys.exit(0)
...
def myfunc( subtraj ):
    ...
    for istate in istates:
        ...
        start = time.time()
        # mb fssh
        params["outfile"]  = "_out_elec_fssh_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"]  = 11 # total number of mb electronic states
        ...
        mb_hot_energy_fssh = ( mb_nbra_namd[:,44] - mb_nbra_namd[:,1] ) * units.au2ev
        #sys.exit(0)
        ...
        start = time.time()
        # mb ida
        params["outfile"] = "_out_elec_ida_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"] = 11 # total number of mb electronic states
        ...
        mb_hot_energy_ida = ( mb_nbra_namd[:,44] - mb_nbra_namd[:,1] ) * units.au2ev

        ### mSDM
        params["decoherence_method"] = 2
        start = time.time()
        # mb msdm
        params["outfile"] = "_out_elec_msdm_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"] = 11 # total number of mb electronic states
        ...
        mb_hot_energy_msdm = ( mb_nbra_namd[:,44] - mb_nbra_namd[:,1] ) * units.au2ev

        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ HOMO -> LUMO+N traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',      fontsize=10)
        plt.ylabel('Energy, eV',    fontsize=10)
        plt.ylim(0,3.5)
        plt.yticks([0,1,2,3])
        for sd_index in range( 11 ):
        ...
        plt.savefig("Si75H64_elec_Dyn_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)

    # For homo-n -> lumo excitations
    istates = [12,13,14,22]
    for istate in istates:
        ...
        start = time.time()
        #sd fssh
        params["outfile"]  = "_out_hole_fssh_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"]  = 22  # total number of sd electronic states
        ...
        sd_hot_energy_fssh = ( sd_nbra_namd[:,77] - sd_nbra_namd[:,1] ) * units.au2ev
        end = time.time()
        ...
        start = time.time()
        #sd ida
        params["outfile"] = "_out_hole_ida_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"] = 22 # total number of sd electronic states
        ...
        sd_hot_energy_ida = ( sd_nbra_namd[:,77] - sd_nbra_namd[:,1] ) * units.au2ev
        end = time.time()
        print("Time to run NBRA NAMD = ", end - start)
        ...
        start = time.time()
        #sd msdm
        params["outfile"] = "_out_hole_msdm_subtraj_"+str(subtraj)+"_istate"+str(istate)+".txt"
        params["nstates"] = 22 # total number of sd electronic states
        ...
        sd_hot_energy_msdm = ( sd_nbra_namd[:,77] - sd_nbra_namd[:,1] ) * units.au2ev
        end = time.time()
        print("Time to run NBRA NAMD = ", end - start)

        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ HOMO-N -> LUMO traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',      fontsize=10)
        plt.ylabel('Energy, eV',    fontsize=10)
        plt.ylim(0,3.5)
        plt.yticks([0,1,2,3])
        for sd_index in range( 22 ):
        ...
        plt.tight_layout()
        plt.legend(fontsize=8, ncol=3, loc="lower left")
        plt.savefig("Si75H64_hole_Dyn_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)
