...
params["nfiles"]         = 4000
params["nstates"]        = 51 # total number of electronic states
params["init_times"]     = [2000]
...
params["nfiles"]         = 4000
params["nstates"]        = 85 # total number of electronic states
params["init_times"]     = [2000]
params["active_space"]   = list(range(params["nstates"])) # indexing is from 0!
# Include HOMO and up to the last electronic state
...
params = {}
params["data_set_paths"] = []
params["data_set_paths"].append(absolute_path+"/../../step3/electron_only/res/")
params["Hvib_re_prefix"] = "Hvib_sd_"; params["Hvib_re_suffix"] = "_re"
params["Hvib_im_prefix"] = "Hvib_sd_"; params["Hvib_im_suffix"] = "_im"
params["nfiles"]         = 4000
params["nstates"]        = 12 # total number of electronic states
params["init_times"]     = [0]
params["active_space"]   = list(range(params["nstates"])) # indexing is from 0!
# Include HOMO and up to the last electronic state
...
params = {}
params["data_set_paths"] = []
params["data_set_paths"].append(absolute_path+"/../../step3/hole_only/res/")
params["Hvib_re_prefix"] = "Hvib_sd_"; params["Hvib_re_suffix"] = "_re"
params["Hvib_im_prefix"] = "Hvib_sd_"; params["Hvib_im_suffix"] = "_im"
params["nfiles"]         = 4000
...
def myfunc( subtraj ):
    ...
    for istate in istates:
        ...
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ Many-body traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        ...
        plt.savefig("Si75H64_mb_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)

        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ Single-Particle traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',   fontsize=10)
        plt.ylabel('Energy, eV', fontsize=10)
        plt.ylim(0,3.0)
        plt.yticks([0,1,2,3])
        ...
        plt.tight_layout()
        plt.legend(fontsize=8, ncol=3, loc="lower left")
        plt.savefig("Si75H64_mixed_sd_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)

    # Electrons
    istates = [5,6,7,8,9,10]
    for istate in istates:

        ### FSSH
        params["decoherence_method"] = 0
        outfile = "_out_si75h64_elec_sd_nbra_namd_fssh_"+str(subtraj)+"_"+str(istate)+".txt"
        elec_sd_nbra_fssh = run_nbra_namd_wrapper( hvib_elec_sd_subtrajs[ subtraj ], istate, outfile, params )

        ### ID-A
        params["decoherence_method"] = 1
        outfile = "_out_si75h64_elec_sd_nbra_namd_ida_"+str(subtraj)+"_"+str(istate)+".txt"
        elec_sd_nbra_ida = run_nbra_namd_wrapper( hvib_elec_sd_subtrajs[ subtraj ], istate, outfile, params )

        ### mSDM
        params["decoherence_method"] = 2
        outfile = "_out_si75h64_elec_sd_nbra_namd_msdm_"+str(subtraj)+"_"+str(istate)+".txt"
        elec_sd_nbra_msdm = run_nbra_namd_wrapper( hvib_elec_sd_subtrajs[ subtraj ], istate, outfile, params )

        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ e$^{-}$ traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',   fontsize=10)
        plt.ylabel('Energy, eV', fontsize=10)
        plt.ylim(0,3.0)
        plt.yticks([0,1,2,3])
        ...
        plt.tight_layout()
        plt.legend(fontsize=8, ncol=3, loc="lower left")
        plt.savefig("Si75H64_elec_sd_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)

    # Hole
    istates = [13,14,15,16,17,18]
    for istate in istates:

        ### FSSH
        params["decoherence_method"] = 0
        outfile = "_out_si75h64_hole_sd_nbra_namd_fssh_"+str(subtraj)+"_"+str(istate)+".txt"
        hole_sd_nbra_fssh = run_nbra_namd_wrapper( hvib_hole_sd_subtrajs[ subtraj ], istate, outfile, params )

        ### ID-A
        params["decoherence_method"] = 1
        outfile = "_out_si75h64_hole_sd_nbra_namd_ida_"+str(subtraj)+"_"+str(istate)+".txt"
        hole_sd_nbra_ida = run_nbra_namd_wrapper( hvib_hole_sd_subtrajs[ subtraj ], istate, outfile, params )

        ### mSDM
        params["decoherence_method"] = 2
        outfile = "_out_si75h64_hole_sd_nbra_namd_msdm_"+str(subtraj)+"_"+str(istate)+".txt"
        hole_sd_nbra_msdm = run_nbra_namd_wrapper( hvib_hole_sd_subtrajs[ subtraj ], istate, outfile, params )

        plt.figure(num=None, figsize=(3.21, 2.41), dpi=300, edgecolor='black', frameon=True)
        plt.subplot(1,1,1)
        plt.title('Si$_{75}$H$_{64}$ h$^{+}$ traj'+str(subtraj)+' istate'+str(istate)+'', fontsize=10)
        plt.xlabel('Time, fs',   fontsize=10)
        plt.ylabel('Energy, eV', fontsize=10)
        plt.ylim(0,3.0)
        plt.yticks([0,1,2,3])
        ...
        plt.tight_layout()
        plt.legend(fontsize=8, ncol=3, loc="lower left")
        plt.savefig("Si75H64_hole_sd_"+str(subtraj)+"_"+str(istate)+".png", dpi=300)
