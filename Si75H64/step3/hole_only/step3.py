...
####################
# 2.0. Generate the homo-n -> lumo excitations. We know before hand that homo-21 -> lumo is to be the highest considered excitation. Spin-polarization is not considered in this study, so excitation of only 1 spin type (alpha herein) is chosen. These "excitations" are in the format of how CP2K would output them. We do this because the routine used below can transform this format of "excitations" into a format expected by Libra
unique_SDs = []
unique_SDs.append(['31, 31'])
for i in range(31,10,-1):
    unique_SDs.append(['%d, 32'%i])
print(unique_SDs)
#sys.exit(0)
